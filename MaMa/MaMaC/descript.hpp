...
namespace bytecode {
  ...
  public:
    ...
    void emit(ofstream &os) const {
      const U4 w0le = eConvertFromTo(w0, eInfo.native, eLittle);
      if (!os.write(reinterpret_cast<const char*>(&w0le), sizeof(w0le)))
        throw EWriteError(__FILE__, __LINE__);
      if (!os.write(reinterpret_cast<const char*>(&w1), sizeof(w1)))
        throw EWriteError(__FILE__, __LINE__);
    }

    explicit Descriptor(const bool dbg = false)
      : w0(eInfo.native << 31 | dbg << 29), w1(0) {}
